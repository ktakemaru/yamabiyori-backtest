...
### 10.5 富士山の日照時間は「過去の気象データ検索」(官署系) に夏季だけ入っている (2026-09-19 実測)

- URL: `https://www.data.jma.go.jp/stats/etrn/view/hourly_s1.php?prec_no=50&block_no=47639&year=YYYY&month=M&day=D&view=`
  (富士山 = prec_no 50 (山梨), block_no 47639, 官署クラス s1)。HTML (API 無し)。生ページは `probe/raw/etrn_fuji_*.html`。
- 列 (実測ヘッダ): 時 | 気圧(現地・海面) | 降水量 | 気温 | 露点温度 | 蒸気圧 | 湿度 | 風向・風速 | **日照時間(h)** | 全天日射量 | 雪 | 天気 | 雲量 | 視程。
  富士山で値が入るのは 現地気圧・気温・露点・蒸気圧・湿度・(夏季のみ) 日照時間。風速・風向・全天日射・天気・雲量・視程は空。
- 日照時間 (06〜18 時) の有無:
  ...
  | 2026-06-20 | × |
  | 2026-07-01 | × |
  | 2026-07-20 | **あり** (1.0, 1.0, 1.0, 1.0, 0.9, 0.8 …) |
  | 2026-08-15 | あり |
  | 2026-08-31 | あり |
  | 2026-09-01 | × |
  | 2026-09-08 / 09-11 / 09-15 | × |
  | 2025-08-15 | あり |
  | 2025-09-15 | × |

  → **7 月上旬〜8 月末だけ観測されている (夏季運用)**。AMeDAS 配信系 (amedastable の日照ビット=0, map の `[null,5]`) に
  日照が無いのはこのため。正確な開始・終了日は未確認 (7/1 は ×、7/20 はあり)。
- 取得経路 (実装済): `python -m backtest.jma_etrn --station fuji --start 2026-07-01 --end 2026-08-31` が 1 日 1 ページ・2 秒間隔で
